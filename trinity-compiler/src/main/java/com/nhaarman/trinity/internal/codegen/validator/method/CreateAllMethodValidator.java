...
class CreateAllMethodValidator implements Validator<RepositoryMethod> {
  ...
  public ProcessingStepResult validate(@NotNull final RepositoryMethod method, @NotNull final ValidationHandler validationHandler) {
    ...
    if (!void.class.getName().equals(returnType)) {
      validationHandler.onError(method.getElement(), null, Message.CREATE_ALL_METHOD_MUST_RETURN_VOID);
      return ERROR;
    }
